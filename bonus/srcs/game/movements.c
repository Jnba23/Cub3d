...
/*   Updated: 2025/02/18 11:05:35 by hmoukit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_move(t_game *game, char dir, int i)
{
	if (dir == 'U' || dir == 'D')
	{
		if (dir == 'U')
	...
	if (dir == 'L' || dir == 'R')
	{
		if (dir == 'L')
			...
			game->pl_inf->turn_dir = -1;
		update_map_l_r(game, i);
	}
	game->render = true;
}

void	update_map_l_r(t_game *game, int i)
{
	float	x;
	float	y;
	float	p_step;
...
void	update_map_u_d(t_game *game, int i)
{
	float	x;
	float	y;
	float	p_step;
